...
def handle_message(user_input):
    ...
    if isinstance(response, dict) and "quiz_id" in response:
        in_quiz = True
        ...
        return render_chat(), q["question"], gr.update(choices=options), gr.update(visible=True), "", ""

    chat_history.append((user_input, response))
    return render_chat(), "", gr.update(choices=[]), gr.update(visible=False), "", ""
...
with gr.Blocks() as demo:
    gr.Markdown("## 🧠 YouTube Assistant with Quiz Support")
    ...
    send_btn.click(fn=handle_message, inputs=user_input,
                   outputs=[chatbot, question_text, option_radio, quiz_section, quiz_feedback, user_input])
